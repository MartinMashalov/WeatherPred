...
from research.experiments.e025_acquire_nbh_v2 import (
    AcquisitionPause,
    BoundedClient,
    acquire_object,
    authorize_full,
...
def config():
    value = json.loads(Path("config/e025_physical_baseline_v2.json").read_text())
    value.update(request_interval_seconds=0, minimum_free_disk_bytes=0)
    value.update(prior_payload_bytes=0, reuse_sources=[])
    return value
...
def test_registered_prior_response_reused_without_relabeling_receipt_or_network(tmp_path):
    archive = Archive(tmp_path)
    cfg = config()
    cfg["prior_payload_bytes"] = 1234
    signature = sha(
        canonical(
            {
                "url": cfg["bucket"] + "fixed",
                "params": None,
                "headers": {"If-Match": '"pinned"', "Range": "bytes=5-7"},
                "cap": 3,
            }
        ).encode()
    )
    ...
    metadata = {
        ...
        "raw_sha256": sha(b"abc"),
    }
    identity = f"{cfg['prior_registration_id']}:fixed:range:5:7"
    rec = archive.append("e025_response", identity, received, metadata, b"abc")
    ...
    cfg["reuse_sources"] = [
        {
            ...
            "request_signature": signature,
            "etag": '"pinned"',
            "last_modified": object_headers()["last-modified"],
            "actual_received_at": iso(received),
        }
    ]
    calls = []
    client = BoundedClient(
        archive, 10, cfg, transport=httpx.MockTransport(lambda request: calls.append(request))
    )
    body, actual_meta, actual_rec = client.get("fixed", "range:5:7", obj=fixed_object(), byte_range=(5, 7))
    assert body == b"abc" and actual_rec == rec and actual_meta == metadata
    assert not calls and client.used == 1234
    cfg["reuse_sources"][0]["actual_received_at"] = "2000-01-01T00:00:00Z"
    with pytest.raises(ValueError, match="provenance changed"):
        client.get("fixed", "range:5:7", obj=fixed_object(), byte_range=(5, 7))
    client.close()
    archive.close()
...
def test_excess_stream_bytes_remain_charged_and_archived(tmp_path):
    ...
    row = archive.latest("e025v2_response", "101:fixed:range")
...
def test_ignored_range_never_reads_full_response_body(tmp_path):
    ...
    records = archive.db.execute("SELECT * FROM records WHERE kind='e025v2_response'").fetchall()
...
def test_interrupted_intent_remains_charged_and_budget_refuses_network(tmp_path):
    archive = Archive(tmp_path)
    archive.append(
        "e025v2_request_started", "3:interrupted", utcnow(), {"reserved_payload_bytes": 65536}, b""
    )
    cfg = config()
...
def test_bounded_chunked_listing_and_stop_file(tmp_path):
    ...
    (tmp_path / "STOP_E025V2_NBH").touch()
...
def test_attempt_limit_survives_interrupted_invocations(tmp_path):
    ...
    for _ in range(3):
        archive.append(
            "e025v2_request_started", "32:fixed:listing", utcnow(), {"reserved_payload_bytes": 65536}, b""
        )
    called = []
    client = BoundedClient(
