...
from research.probes.bankroll_dataset_v2 import (
...
# The four categorical fields are a source-specific metadata repair, not a
# general permission to coerce nonnumeric temperatures or discard contracts.
from research.probes.bankroll_dataset_v2 import (
    CATEGORICAL_CHECKPOINT_ID,
    CATEGORICAL_EVENT,
    CATEGORICAL_SOURCE_ID,
    CATEGORICAL_SOURCE_SHA256,
    CATEGORICAL_TICKERS,
)


def categorical_fixture(ticker):
    raw = market(ticker)
    raw.update(
        event_ticker=CATEGORICAL_EVENT,
        open_time="2026-04-10T15:00:00Z",
        close_time="2026-04-12T04:59:00Z",
        settlement_ts="2026-04-12T13:01:45Z",
        result="no",
        expiration_value="No",
    )
    source = {
        "available_at": RECEIPT,
        "source_record_id": CATEGORICAL_SOURCE_ID,
        "body_sha256": CATEGORICAL_SOURCE_SHA256,
    }
    windows = {"window": WINDOWS["window"], "series": {"KXHIGHMIA": {"standard_utc_offset_hours": -5}}}
    return raw, source, windows


@pytest.mark.parametrize("ticker", sorted(CATEGORICAL_TICKERS))
@pytest.mark.parametrize("status", ["finalized", "settled"])
def test_each_pinned_categorical_no_keeps_official_cash_result_without_numeric_claim(ticker, status):
    raw, source, windows = categorical_fixture(ticker)
    raw["status"] = status
    normalized = market_metadata(
        raw, CATEGORICAL_EVENT, source, windows, checkpoint_record_id=CATEGORICAL_CHECKPOINT_ID
    )
    assert normalized["expiration_value"] == "No"
    assert normalized["numeric_corroboration"] == "categorical_result_only"
    assert normalized["outcome"] == 0
    assert normalized["reported_result"] == "no"
    assert normalized["settled_ts"] is not None
    assert normalized["metadata_provenance"] == source


@pytest.mark.parametrize(
    "field,value",
    [
        ("ticker", CATEGORICAL_EVENT + "-B80.5"),
        ("result", "yes"),
        ("result", ""),
        ("status", "active"),
        ("settlement_ts", None),
...
def test_categorical_exception_never_expands_ticker_or_result_status_scope(field, value):
    raw, source, windows = categorical_fixture(min(CATEGORICAL_TICKERS))
    raw[field] = value
    with pytest.raises(ValueError, match="exact declared source/result binding"):
        market_metadata(
            raw, CATEGORICAL_EVENT, source, windows, checkpoint_record_id=CATEGORICAL_CHECKPOINT_ID
        )
...
@pytest.mark.parametrize(
    "source_id,source_hash,checkpoint",
    [
        (CATEGORICAL_SOURCE_ID + 1, CATEGORICAL_SOURCE_SHA256, CATEGORICAL_CHECKPOINT_ID),
        (CATEGORICAL_SOURCE_ID, "0" * 64, CATEGORICAL_CHECKPOINT_ID),
        (CATEGORICAL_SOURCE_ID, CATEGORICAL_SOURCE_SHA256, CATEGORICAL_CHECKPOINT_ID + 1),
        (CATEGORICAL_SOURCE_ID, CATEGORICAL_SOURCE_SHA256, None),
    ],
)
def test_categorical_source_hash_record_and_checkpoint_are_all_required(source_id, source_hash, checkpoint):
    raw, source, windows = categorical_fixture(min(CATEGORICAL_TICKERS))
    source.update(source_record_id=source_id, body_sha256=source_hash)
    with pytest.raises(ValueError, match="exact declared source/result binding"):
        market_metadata(raw, CATEGORICAL_EVENT, source, windows, checkpoint_record_id=checkpoint)


@pytest.mark.parametrize(
    "value", ["Yes", "no", "NO", " No", "No ", "N/A", " ", True, [], {}, "NaN", "Infinity", "-Infinity"]
)
def test_unregistered_categorical_and_nonfinite_expiration_values_stay_errors(value):
    raw, source, windows = categorical_fixture(min(CATEGORICAL_TICKERS))
    raw["expiration_value"] = value
    with pytest.raises(ValueError, match="nonnumeric|Nonfinite"):
        market_metadata(
            raw, CATEGORICAL_EVENT, source, windows, checkpoint_record_id=CATEGORICAL_CHECKPOINT_ID
        )
...
def test_missing_numeric_corroboration_behavior_unchanged(value):
    raw = market()
    raw["expiration_value"] = value
    normalized = market_metadata(raw, EVENT, {"available_at": RECEIPT, "source_record_id": 1}, WINDOWS)
    assert normalized["outcome"] == 1
    assert normalized["expiration_value"] == value
    assert normalized["numeric_corroboration"] == "missing"


def test_pinned_ticker_still_rejects_a_numeric_temperature_contradicting_its_official_no():
    raw, source, windows = categorical_fixture(min(CATEGORICAL_TICKERS))
    raw["expiration_value"] = "31"
    with pytest.raises(ValueError, match="contradicts primary contract predicate"):
        market_metadata(
            raw, CATEGORICAL_EVENT, source, windows, checkpoint_record_id=CATEGORICAL_CHECKPOINT_ID
        )
    raw["expiration_value"] = "32"
    ...
    assert normalized["numeric_corroboration"] == "numeric_value_matches_result"
