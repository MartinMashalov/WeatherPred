...
from research.experiments.e024_annual_replay_v2 import (
    CONFIG,
    DAY,
    FIRST,
    LAST,
...
def test_v2_changes_only_explicit_normalization_revision_settings():
    old = json.loads(Path("config/e024_annual_replay.json").read_bytes())
    current = settings()
    assert current["experiment"] == "E024-restricted-annual-walk-forward-v2"
    assert {k: current[k] for k in old if k != "experiment"} == {
        k: v for k, v in old.items() if k != "experiment"
    }
    revision = current["normalization_revision"]
    assert (revision["source_cutoff_id"], revision["source_cutoff_at"]) == (
        123234,
        "2026-09-06T19:48:05.160651+00:00",
    )
    assert revision["expected_contracts"] == 10416 and revision["expected_categorical_cases"] == 4
    current["cpu_threads"] = 3
    policies = candidates(json.loads(Path(current["parent_policy_config"]).read_bytes()))
    with pytest.raises(ValueError, match="cannot change the frozen trading policy"):
        config_check(current, policies)


@pytest.mark.parametrize(
    "identifier,at",
    [(123235, "2026-09-06T19:48:05.160651+00:00"), (123234, "2026-09-06T19:48:06.160651+00:00")],
)
def test_v2_different_source_cutoff_rejected_before_archive_access(identifier, at):
    from research.experiments.e024_annual_replay_v2 import prepare_dataset

    with pytest.raises(ValueError, match="original pre-diagnostic archive cutoff"):
        prepare_dataset(None, settings(), identifier, at)
