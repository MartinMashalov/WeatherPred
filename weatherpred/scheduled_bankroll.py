"""One continuous account with a predeclared chronological policy schedule.

Explicit fork of bankroll_replay.replay, parent SHA256 below. The accounting,
execution, release, exposure and drawdown code is preserved. Only decision-time
policy/risk gates and schedule metadata are added. The original frozen module
is never modified; its immutable money/provenance helpers are reused.

This module neither selects a strategy nor establishes historical availability.
The caller must register the schedule and provide its chronological selection
receipts before any corresponding period is evaluated.
"""
...
from weatherpred.bankroll_replay import (
    ZERO,
    _gate,
    _provenance_time,
    _quantity,
...
PARENT_KERNEL_SHA256 = "27a6c46c309dde245f26bb52f359129495edda6321d89140afe77e9bf54405c2"
E023_RISK_FRACTIONS = frozenset(decimal(value) for value in (".005", ".01", ".025", ".05"))


def _schedule_fraction(value):
    try:
        return decimal(value)
    except InvalidOperation as error:
        raise ValueError("A schedule risk fraction must be finite and numeric") from error


def validate_schedule(schedule, config, start_ts, end_ts):
    """Validate the full fixed schedule without selecting or reading outcomes."""
    start, end = timestamp(start_ts), timestamp(end_ts)
    if end <= start:
        raise ValueError("Replay end must follow its start")
    policies = config.get("allowed_policy_ids")
    if (
        not isinstance(policies, (list, tuple))
        or any(not isinstance(policy, str) or not policy or policy != policy.strip() for policy in policies)
        or len(set(policies)) != len(policies)
    ):
        raise ValueError("allowed_policy_ids must be unique nonempty string IDs")
    supplied = config.get("allowed_risk_fractions")
    if not isinstance(supplied, (list, tuple)) or not supplied:
        raise ValueError("An explicit allowed_risk_fractions list is required")
    fractions = [_schedule_fraction(value) for value in supplied]
    if len(set(fractions)) != len(fractions) or not set(fractions) <= E023_RISK_FRACTIONS:
        raise ValueError("Allowed fractions must be a unique subset of the four E023 risk fractions")
    if not isinstance(schedule, (list, tuple)) or not schedule:
        raise ValueError("A nonempty chronological policy schedule is required")
    normalized = []
    for entry in schedule:
        if not isinstance(entry, dict) or set(entry) != {"effective_ts", "policy_id", "risk_fraction"}:
            raise ValueError("A regime needs exactly effective_ts, policy_id, and risk_fraction")
        effective = timestamp(entry["effective_ts"])
        if not start <= effective < end:
            raise ValueError("Schedule effective times must lie inside the replay window")
        if not normalized and effective != start:
            raise ValueError("The first regime must be effective exactly at replay start")
        if normalized and effective <= normalized[-1]["effective_ts"]:
            raise ValueError("Schedule effective times must be strictly increasing and unique")
        policy, fraction = entry["policy_id"], entry["risk_fraction"]
        if policy is None:
            if fraction is not None:
                raise ValueError("Cash requires policy_id=None and risk_fraction=None")
            value = None
        else:
            if not isinstance(policy, str) or policy not in policies:
                raise ValueError("A selected policy must belong to allowed_policy_ids")
            value = _schedule_fraction(fraction)
            if value not in fractions:
                raise ValueError("A selected risk fraction is not explicitly allowed")
            value = str(value)
        normalized.append({"effective_ts": effective, "policy_id": policy, "risk_fraction": value})
    return normalized


def replay_scheduled(trades, config, schedule, start_ts, end_ts):
    """Replay one flat-start account under a predeclared policy/risk schedule.

    Schedule changes govern new decisions only. Existing pending orders retain
    their original quantities/limits; filled holdings retain their original exit
    instructions. Cash, exposure, fees and drawdown state never reset at a switch.

    Config requires allowed_policy_ids and allowed_risk_fractions in addition to
    the parent kernel's execution/exposure settings. The parent's risk_fraction
    is ignored: only the decision-time schedule supplies the sizing fraction.
    Every order requires policy_id. The first schedule entry is effective exactly
    at start_ts; every later entry lies strictly before end_ts. Cash entries use
    policy_id=None and risk_fraction=None. The parent accounting result is
    unchanged in shape except for the added schedule_metadata field.
    """
    start, end = timestamp(start_ts), timestamp(end_ts)
    if end <= start:
        raise ValueError("Replay end must follow its start")
    regimes = validate_schedule(schedule, config, start, end)
    regime_times = [regime["effective_ts"] for regime in regimes]
    decision_regimes = []
    ...
    fractions = {
        ...
        for key in ("max_event_fraction", "max_cluster_fraction", "max_total_fraction")
    }
    ...
    for original in trades:
        ...
        seen.add(identifier)
        if trade.get("policy_id") not in config["allowed_policy_ids"]:
            raise ValueError("Each intent must name an explicitly allowed policy_id")
    ...
    def decide(identifier, when):
        ...
        regime = regimes[bisect_right(regime_times, when) - 1]
        decision_regimes.append(
            {
                "trade_id": identifier,
                "decision_ts": when,
        ...
        if killed:
            ...
            return
        if regime["policy_id"] is None:
            reject(identifier, when, "scheduled_cash", "decision")
            return
        if trade["policy_id"] != regime["policy_id"]:
            reject(identifier, when, "policy_not_selected_at_decision", "decision")
            return
        risk_fraction = decimal(regime["risk_fraction"])
        ...
        bounds = {
            "cash_limit": cash,
            "risk_fraction_limit": risk_fraction * equity,
    ...
    return {
        "schedule_metadata": {
            "parent_kernel_sha256": PARENT_KERNEL_SHA256,
            "regimes": regimes,
            "allowed_policy_ids": list(config["allowed_policy_ids"]),
            "allowed_risk_fractions": [str(decimal(value)) for value in config["allowed_risk_fractions"]],
            "decision_regimes": decision_regimes,
            "trade_policy_ids": {identifier: trade["policy_id"] for identifier, trade in by_id.items()},
            "switch_behavior": "New decisions use their effective policy and fraction; prior reservations, holdings, exits, fees, exposure and drawdown persist.",
            "cash_initializations": 1,
            "schedule_selection_performed": False,
        },
