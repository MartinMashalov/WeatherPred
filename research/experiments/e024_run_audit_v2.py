...
AUDIT_SOURCES = [
    ...
    Path("tests/test_e024_run_audit_v2.py"),
...
def validate_protocol(protocol):
    ...
    if config["experiment"] != "E024-restricted-annual-walk-forward-v2":
    ...
    if any(config.get(key) != value for key, value in baseline.items() if key != "experiment"):
        raise ValueError("The metadata revision changed an original scientific or resource setting")
    if (
        protocol["source_cutoff_id"] != 123234
        or protocol["source_cutoff_at"] != "2026-09-06T19:48:05.160651+00:00"
    ):
        raise ValueError("The metadata revision changed the original source cutoff")
...
def register_audit(archive, run_id, replay_bootstrap=False):
    ...
    protocol = {
        "experiment": "E024-independent-full-account-audit-v2",
...
def audit_run(archive, audit_id):
    _, protocol = checked_record(
        archive, audit_id, "experiment_protocol", "E024-independent-full-account-audit-v2"
    )
