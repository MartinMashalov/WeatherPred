...
CONFIG = Path("config/e025_physical_baseline_v2.json")
HOUR = 3_600_000
NS = {"s": "http://s3.amazonaws.com/doc/2006-03-01/"}
SOURCE_PATHS = [
    "research/experiments/e025_acquire_nbh_v2.py",
    "weatherpred/nbh_v2.py",
    "weatherpred/archive.py",
    "weatherpred/timeutil.py",
    str(CONFIG),
    "tests/test_nbh_v2.py",
    "tests/test_e025_acquire_nbh_v2.py",
    "pyproject.toml",
    "uv.lock",
]
...
def register(archive, config):
    verify_prior_evidence(archive, config)
    ...
    for name in SOURCE_PATHS + [config["manifest_path"]]:
        ...
        source_records[name] = archive.append("e025v2_source", name, utcnow(), {}, body)
    protocol = {
        "config": config,
        ...
        "new_forecast_downloads_before_this_registration": 0,
        "prior_full_forecast_responses_retained": sum(
            item["record_key"].endswith(":full") for item in config["reuse_sources"]
        ),
        "prior_failed_acquisition_acknowledged": True,
        "scoring_authorized": False,
    }
    return archive.append("e025v2_protocol", config["experiment"], utcnow(), {}, canonical(protocol).encode())


def load_protocol(archive, registration_id):
    row = archive.db.execute("SELECT * FROM records WHERE id=?", (registration_id,)).fetchone()
    if row is None or row["kind"] != "e025v2_protocol":
        raise ValueError("Expected an E025 registration record")
    protocol = json.loads(checked_body(archive, row))
    verify_prior_evidence(archive, protocol["config"])
...
def verify_prior_evidence(archive, config):
    report = archive.db.execute(
        "SELECT * FROM records WHERE id=?", (config["prior_report_record_id"],)
    ).fetchone()
    if (
        report is None
        or report["kind"] != "e025_report"
        or report["body_sha256"] != config["prior_report_sha256"]
    ):
        raise ValueError("Pinned prior failure report changed")
    data = json.loads(checked_body(archive, report))
    if (
        data["registration_id"] != config["prior_registration_id"]
        or data["charged_payload_bytes_including_interrupted_reservations"] != config["prior_payload_bytes"]
    ):
        raise ValueError("Prior lineage payload accounting differs")
    for item in config["reuse_sources"]:
        row = archive.db.execute("SELECT * FROM records WHERE id=?", (item["record_id"],)).fetchone()
        if (
            row is None
            or row["kind"] != "e025_response"
            or row["key"] != item["record_key"]
            or row["record_sha256"] != item["record_sha256"]
            or row["body_sha256"] != item["body_sha256"]
        ):
            raise ValueError("Pinned reusable response changed")
        checked_body(archive, row)
...
class BoundedClient:
    ...
    def restore_budget(self):
        ...
        rows = self.archive.db.execute(
            "SELECT * FROM records WHERE kind IN ('e025v2_request_started','e025v2_response') AND key LIKE ?",
            (self.prefix + "%",),
        )
        for row in rows:
            meta = json.loads(row["metadata"])
            if row["kind"] == "e025v2_request_started":
        ...
        return self.config["prior_payload_bytes"] + sum(
            finished.get(identity, reserved) for identity, reserved in starts.items()
        )
    ...
    def get(self, key, role, *, params=None, obj=None, byte_range=None):
        ...
        prior_key = f"{self.config['prior_registration_id']}:{key}:{role}"
        reusable = [item for item in self.config["reuse_sources"] if item["record_key"] == prior_key]
        if reusable:
            if len(reusable) != 1:
                raise ValueError("Ambiguous registered prior response")
            pinned = reusable[0]
            prior = self.archive.db.execute(
                "SELECT * FROM records WHERE id=?", (pinned["record_id"],)
            ).fetchone()
            if prior is None or prior["kind"] != "e025_response" or prior["key"] != prior_key:
                raise ValueError("Prior response identity changed")
            if (
                prior["body_sha256"] != pinned["body_sha256"]
                or prior["record_sha256"] != pinned["record_sha256"]
            ):
                raise ValueError("Prior response hash changed")
            meta = json.loads(prior["metadata"])
            if (
                not meta["complete"]
                or meta["request_signature"] != signature
                or signature != pinned["request_signature"]
            ):
                raise ValueError("Prior response request signature changed")
            if (
                meta["headers"].get("etag") != pinned["etag"]
                or meta["headers"].get("last-modified") != pinned["last_modified"]
                or meta["actual_received_at"] != pinned["actual_received_at"]
            ):
                raise ValueError("Prior response provenance changed")
            body = checked_body(self.archive, prior)
            validate_headers(meta["status"], meta["headers"], obj, byte_range, cap)
            if len(body) > cap:
                raise ValueError("Prior payload exceeds registered ceiling")
            return body, meta, prior["id"]
        cached = self.archive.latest("e025v2_response", identity)
        ...
        prior_attempts = self.archive.db.execute(
            "SELECT count(*) FROM records WHERE kind='e025v2_request_started' AND key=?",
            (identity,),
        ).fetchone()[0]
        for attempt in range(prior_attempts, self.config["maximum_attempts_per_request"]):
            ...
            rec = self.archive.append("e025v2_response", identity, received, metadata, bytes(body))
...
def authorize_full(client, key, seed):
    ...
    existing = client.archive.latest("e025v2_full_authorization", identity)
    if existing:
        return json.loads(checked_body(client.archive, existing))["reason"]
    rows = client.archive.db.execute(
        "SELECT * FROM records WHERE kind='e025v2_full_authorization' AND key LIKE ?",
        (client.prefix + "%",),
    ).fetchall()
    ...
    client.archive.append(
        "e025v2_full_authorization", identity, utcnow(), {}, canonical({"reason": reason}).encode()
    )
    return reason
...
def acquire_object(client, entry, offsets):
    ...
    prior_full_key = f"{client.config['prior_registration_id']}:{entry['key']}:full"
    if any(item["record_key"] == prior_full_key for item in client.config["reuse_sources"]):
        ...
        if set(cards) != stations:
            raise ValueError("Pinned prior full response lacks exact station cards")
        full_reason = "registered_prior_full_response"
    if not cards and stations <= set(offsets):
        try:
...
def run(archive, registration_id, report_dir):
    ...
    try:
        for entry in entries:
            ...
            existing = archive.latest("e025v2_object", identity)
            if existing:
                ...
                record_id = archive.append(
                    "e025v2_object", identity, utcnow(), {}, canonical(result).encode()
                )
    ...
    report = {
        ...
        "prior_registration_id": config["prior_registration_id"],
        "prior_report_record_id": config["prior_report_record_id"],
        "prior_charged_payload_bytes": config["prior_payload_bytes"],
        "new_charged_payload_bytes": client.used - config["prior_payload_bytes"],
        "scoring_authorized": False,
        "scores_computed": False,
        "historical_public_availability_verified": False,
    }
    verify_source_hashes(protocol)
    rec = archive.append("e025v2_report", str(registration_id), utcnow(), {}, canonical(report).encode())
    report["report_record_id"] = rec
    report_dir.mkdir(parents=True, exist_ok=True)
    destination = report_dir / f"E025V2_acquisition_{registration_id}.json"
...
def main():
    ...
    if args.prepare:
        ...
        path = directory / "E025V2_acquisition_manifest.json"
    ...
    try:
        with (archive.root / "e025v2_nbh.lock").open("a+") as lock:
            try:
