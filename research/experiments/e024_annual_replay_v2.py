...
from research.probes.bankroll_dataset_v2 import ArchiveReader, build_dataset, expected_events
...
CONFIG = Path("config/e024_annual_replay_v2.json")
DAY = 86400
...
SOURCES = [
    ...
    Path("research/experiments/e024_annual_replay.py"),
    Path("config/e024_annual_replay.json"),
    Path("research/probes/e024_expiration_diagnostic.py"),
    Path("tests/test_bankroll_dataset_v2.py"),
    Path("tests/test_e024_annual_replay_v2.py"),
    Path("research/probes/bankroll_dataset_v2.py"),
...
def config_check(config, policies):
    ...
    baseline = json.loads(Path("config/e024_annual_replay.json").read_bytes())
    unchanged = {
        k: v
        for k, v in config.items()
        if k not in ("experiment", "normalization_revision", "revision_evidence")
    }
    if unchanged != {k: v for k, v in baseline.items() if k != "experiment"}:
        raise ValueError("V2 cannot change the frozen trading policy, math, dates or limits")
...
def dataset_gate(dataset, config):
    ...
    revision = config["normalization_revision"]
    if (
        dataset.get("source_cutoff_id") != revision["source_cutoff_id"]
        or dataset.get("source_cutoff_at") != revision["source_cutoff_at"]
        or dataset.get("contracts") != revision["expected_contracts"]
    ):
        raise ValueError("V2 dataset changed the fixed cutoff or complete contract census")
    categorical = [
        m["metadata"]
        for m in dataset["markets"]
        if m["metadata"].get("numeric_corroboration") == "categorical_result_only"
    ]
    if (
        len(categorical) != revision["expected_categorical_cases"]
        or sorted(m["ticker"] for m in categorical) != revision["tickers"]
    ):
        raise ValueError("V2 categorical exception census differs from the four declared contracts")
    for metadata in categorical:
        if (
            metadata["event"] != revision["event"]
            or metadata["expiration_value"] != revision["allowed_expiration_value"]
            or metadata["reported_result"] != revision["official_result"]
            or metadata["reported_status"] not in ("finalized", "settled")
            or metadata["outcome"] != 0
            or metadata["settled_ts"] is None
            or metadata["checkpoint_record_id"] != revision["checkpoint_record_id"]
            or metadata["metadata_provenance"]["source_record_id"] != revision["metadata_record_id"]
            or metadata["metadata_provenance"]["body_sha256"] != revision["metadata_body_sha256"]
        ):
            raise ValueError("V2 categorical normalized source/result binding changed")
    events = set()
    tickers = set()
...
def prepare_dataset(archive, config, cutoff_id, cutoff_at):
    revision = config["normalization_revision"]
    if cutoff_id != revision["source_cutoff_id"] or cutoff_at != revision["source_cutoff_at"]:
        raise ValueError("V2 must retain the original pre-diagnostic archive cutoff")
    for evidence in config["revision_evidence"]:
        row = archive.db.execute("SELECT * FROM records WHERE id=?", (evidence["id"],)).fetchone()
        if row is None or row["kind"] != evidence["kind"] or row["body_sha256"] != evidence["body_sha256"]:
            raise ValueError("V2 preparation failure/diagnostic lineage changed")
        read_json_record(archive, row)
...
def register(archive, dataset_id):
    ...
    protocol = {
        ...
        "revision_evidence": config["revision_evidence"],
...
def load_registration(archive, identifier, load_dataset=True):
    ...
    if protocol.get("config", {}).get("experiment") != "E024-restricted-annual-walk-forward-v2":
...
def supervised_execute(args):
    ...
    try:
        ...
        command = [
            sys.executable,
            "-m",
            "research.experiments.e024_annual_replay_v2",
            "--worker",
            "--run-record-id",
            key,
            "--archive-root",
