...
CONFIG = Path("config/e027_prospective_capture_v2.json")
SOURCE_PATHS = [
    "research/experiments/e027_prospective_capture_v2.py",
    str(CONFIG),
    "tests/test_e027_prospective_capture_v2.py",
...
def read_record(archive, row):
    fields = [
        row[key] for key in ("kind", "key", "available_at", "metadata", "body_sha256", "previous_sha256")
    ]
    ...
    if (
        digest(body) != row["body_sha256"]
        or digest(canonical(fields).encode()) != row["record_sha256"]
        or row["previous_sha256"] != (previous[0] if previous else "0" * 64)
    ):
        raise ValueError("Archived bytes, record hash or predecessor link changed")
...
def validate_config(config):
    ...
    extras = sorted(["KMDW", "KFLL", "KFXE", "KOPF", "KPMP", "KTMB", "KHWO", "KHST"])
    expected = sorted({case["station_id"] for case in json.loads(body)["cases"]} | set(extras))
    if config["extra_station_ids"] != extras or config["stations"] != expected or len(expected) != 28:
        raise ValueError("Expected exact20 E022 stations plus the eight fixed extra airports")
    if (
    ...
    if config["maximum_contracts"] != 1000 or config["book_batch_size"] != 100:
        raise ValueError("Expected exactly1000contract cap and100contract book batches")
    if config["maximum_requests"] != 80000 or config["maximum_payload_bytes"] != 4294967296:
        raise ValueError("Fresh v2 request/payload budget differs from the registered scope")
    if config["start_delay_seconds"] != 30 or config["metadata_refresh_seconds"] != 600:
        raise ValueError("Start delay or metadata refresh cadence differs from fixed protocol")
    if config["prior_registration_id"] != 116258 or config["prior_exclusion_panel_record_id"] != 116657:
        raise ValueError("V2 metadata remediation must reference the exact frozen v1 run and panel")
    if config["lock_file"] != "e027_capture.lock":
        raise ValueError("Both capture versions must use the same singleton lock")


@contextmanager
def singleton_lock(root):
    """The same OS lock as frozen v1 prevents overlapping data collectors."""
    with (Path(root) / "e027_capture.lock").open("a+") as lock:
        try:
            fcntl.flock(lock, fcntl.LOCK_EX | fcntl.LOCK_NB)
        except BlockingIOError:
            raise RuntimeError("Another E027 collector holds the shared singleton lock") from None
        yield


def prior_evidence(archive, config):
    """Read only prior protocol, exclusion metadata and durable usage totals."""
    prior_id, panel_id = config["prior_registration_id"], config["prior_exclusion_panel_record_id"]
    prior_row = archive.db.execute("SELECT * FROM records WHERE id=?", (prior_id,)).fetchone()
    panel_row = archive.db.execute("SELECT * FROM records WHERE id=?", (panel_id,)).fetchone()
    stopped = archive.latest("e027_stopped", str(prior_id))
    if (
        prior_row is None
        or prior_row["kind"] != "e027_protocol"
        or panel_row is None
        or panel_row["kind"] != "e027_market_panel"
        or panel_row["key"] != str(prior_id)
        or stopped is None
        or not prior_id < panel_id < stopped["id"]
    ):
        raise ValueError("V2 requires the pinned v1 exclusion panel and a completed v1 stop record")
    prior = read_record(archive, prior_row)
    summary = read_record(archive, stopped)
    if summary["registration_id"] != prior_id or any(
        summary[key] != 0 for key in ("signals", "scores", "fills", "orders")
    ):
        raise ValueError("Prior collector report differs from its data-only lineage")
    # Hash the complete panel, but decode only the metadata-only market_states
    # array; selected_markets contains values irrelevant to this amendment.
    panel_bytes = archive.body(panel_row)
    fields = [
        panel_row[key]
        for key in ("kind", "key", "available_at", "metadata", "body_sha256", "previous_sha256")
    ]
    before_panel = archive.db.execute(
        "SELECT record_sha256 FROM records WHERE id<? ORDER BY id DESC LIMIT 1", (panel_id,)
    ).fetchone()
    if (
        digest(panel_bytes) != panel_row["body_sha256"]
        or digest(canonical(fields).encode()) != panel_row["record_sha256"]
        or panel_row["previous_sha256"] != (before_panel[0] if before_panel else "0" * 64)
    ):
        raise ValueError("Pinned v1 exclusion metadata changed")
    text = panel_bytes.decode()
    marker = '"market_states":'
    position = text.index(marker) + len(marker)
    states, _ = json.JSONDecoder().raw_decode(text[position:])
    excluded = [state for state in states if state["reason"] == "unsupported_contract_ticker"]
    decimal = [
        state
        for state in excluded
        if isinstance(state["ticker"], str)
        and re.fullmatch(r"[A-Z0-9_-]+(?:\.[0-9]+)?", state["ticker"])
        and "." in state["ticker"]
    ]
    if len(states) != 706 or len(excluded) != 456 or len(decimal) != 456:
        raise ValueError("Prior exclusion panel differs from the declared metadata-only remediation")
    starts, finishes = {}, {}
    for row in archive.db.execute(
        ...
        if row["kind"] == "e027_request_started":
            starts[row["id"]] = metadata["reserved_bytes"]
        else:
            identifier = metadata["request_started_record_id"]
            if identifier in finishes:
                raise ValueError("Duplicate prior response usage record")
            if metadata["payload_bytes"] != (archive.root / "blobs" / row["body_sha256"]).stat().st_size:
                raise ValueError("Prior payload metadata differs from retained byte size")
            finishes[identifier] = metadata["payload_bytes"]
    if not set(finishes) <= set(starts):
        raise ValueError("Prior response usage lacks its durable request reservation")
    usage = {
        "requests": len(starts),
        "charged_payload_bytes": sum(finishes.get(key, reserved) for key, reserved in starts.items()),
    }
    if usage != {
        "requests": summary["requests_total"],
        "charged_payload_bytes": summary["charged_payload_bytes"],
    }:
        raise ValueError("Prior stopped usage totals differ from durable request metadata")
    return {
        "registration_id": prior_id,
        "registration_body_sha256": prior_row["body_sha256"],
        "registration_record_sha256": prior_row["record_sha256"],
        "exclusion_panel_record_id": panel_id,
        "exclusion_panel_body_sha256": panel_row["body_sha256"],
        "exclusion_panel_record_sha256": panel_row["record_sha256"],
        "stopped_record_id": stopped["id"],
        "stopped_body_sha256": stopped["body_sha256"],
        "stopped_record_sha256": stopped["record_sha256"],
        "usage": usage,
        "registered_at": prior["registered_at"],
        "start_at": prior["start_at"],
        "stop_at": prior["stop_at"],
        "actual_stopped_at": summary["finished_at"],
        "observed_contracts": 706,
        "decimal_contracts_excluded": 456,
        "receipts_reused_in_v2": False,
        "prior_backfill_clock_reused": False,
        "v2_request_payload_budget_is_separate": True,
    }


def register(archive, config):
    validate_config(config)
    prior = prior_evidence(archive, config)
    now = utcnow()
    if parse_time(prior["actual_stopped_at"]) > now:
        raise ValueError("A new collection window cannot precede the prior run's stop")
    ...
    protocol = {
        ...
        "run_command_template": [
            sys.executable,
            "-m",
            "research.experiments.e027_prospective_capture_v2",
            "--run-record-id",
            "REGISTRATION_ID",
            "--archive-root",
        ...
        "source_sha256": {},
        "source_record_ids": {},
        "new_public_inputs_before_registration": 0,
        "prior_run": prior,
        "first_local_receipts_start_new_registration": True,
    }
    for path in SOURCE_PATHS + [config["parent_manifest"]]:
        body = Path(path).read_bytes()
        protocol["source_sha256"][path] = digest(body)
        protocol["source_record_ids"][path] = archive.append("e027v2_source", path, now, {}, body)
    rec = archive.append("e027v2_protocol", config["experiment"], now, {}, canonical(protocol).encode())
    return rec, protocol
...
class ReceiptClient:
    def __init__(self, archive, run_id, protocol, *, transport=None):
        ...
        self.client = httpx.Client(
            ...
            headers={
                "User-Agent": "WeatherPred-E027v2/1.0 public receipt research",
                "Accept-Encoding": "identity",
            },
        )
        starts, finishes = {}, {}
        for row in archive.db.execute(
            "SELECT * FROM records WHERE kind IN ('e027v2_request_started','e027v2_http') AND key LIKE ?",
            (f"{run_id}:%",),
        ):
            meta = json.loads(row["metadata"])
            if row["kind"] == "e027v2_request_started":
    ...
    def get(self, path, params, *, role, slot, catalog=False):
        ...
        for attempt in range(self.config["maximum_attempts"]):
            ...
            rec = self.archive.append("e027v2_http", identity, received, metadata, bytes(body))
...
def remember_version(archive, run_id, kind, value, source_id, row_index, received_at, details):
    ...
    existing = archive.latest("e027v2_first_seen", key)
    if existing:
    ...
    rec = archive.append("e027v2_first_seen", key, parse_time(received_at), {}, canonical(event).encode())
    return rec, True
...
def select_contracts(markets, now, cap):
    ...
    for row in sorted(markets, key=lambda row: str(row.get("ticker", ""))):
        ...
        if not isinstance(ticker, str) or not (
            len(ticker) <= 128 and re.fullmatch(r"[A-Z0-9_-]+(?:\.[0-9]+)?", ticker)
        ):
...
def refresh_markets(client, slot):
    ...
    catalog_has_cursor = bool(data.get("cursor"))
    ...
    limits = sorted(
        {
            ...
            if row["reason"] in ("series_cap", "unsupported_series_ticker", "duplicate_series_ticker")
        }
        | {
            row["reason"]
            for row in states
            ...
            in (
                "contract_cap",
                "unsupported_contract_ticker",
                "duplicate_contract_ticker",
                "unsupported_market_times",
                "unsupported_market_status",
            )
        }
        | ({"series_catalog_cursor"} if catalog_has_cursor else set())
        | ({"market_page_or_request_error"} if errors else set())
    )
    result = {
        "slot": slot,
        ...
        "complete_universe_at_refresh": not errors and not limits,
        "universe_limit_reasons": limits,
        "created_at": iso(utcnow()),
        "selection_uses_prices": False,
    }
    rec = client.archive.append(
        "e027v2_market_panel", str(client.run_id), utcnow(), {}, canonical(result).encode()
    )
    return result, rec
...
def capture_index(client, slot):
    ...
    initial = (
        client.archive.db.execute(
            "SELECT 1 FROM records WHERE kind='e027v2_first_seen' AND key LIKE ? LIMIT 1",
...
def run(archive, run_id, report_dir):
    ...
    if row is None or row["kind"] != "e027v2_protocol":
        raise ValueError("Expected an E027v2 registration")
    ...
    for source in archive.db.execute(
        "SELECT * FROM records WHERE kind='e027v2_first_seen' AND key LIKE ?", (f"{run_id}:metar:%",)
    ):
        station_seen.add(read_record(archive, source)["station_id"])
    latest = archive.latest("e027v2_market_panel", str(run_id))
    panel, panel_id = (read_record(archive, latest), latest["id"]) if latest else (None, None)
    started_slots = {
        int(r["key"].split(":")[-1])
        for r in archive.db.execute(
            "SELECT key FROM records WHERE kind='e027v2_cycle_started' AND key LIKE ?", (f"{run_id}:%",)
        )
    }
    ...
    try:
        while True:
            ...
            archive.append("e027v2_cycle_started", f"{run_id}:{slot}", now, {}, canonical(intent).encode())
            last_slot = slot
            ...
            verify_sources(protocol)
            frame_id = archive.append("e027v2_cycle", str(run_id), utcnow(), {}, canonical(frame).encode())
            ...
            progress = {
                ...
                "complete_universe_at_refresh": panel is not None and panel["complete_universe_at_refresh"],
                "universe_limit_reasons": panel["universe_limit_reasons"] if panel else ["no_market_panel"],
                "prior_run_usage": protocol["prior_run"]["usage"],
                "prior_receipts_reused": False,
            ...
            publish(report_dir / f"E027v2_progress_{run_id}.json", progress)
    ...
    completed_slots = {
        ...
        for r in archive.db.execute(
            "SELECT * FROM records WHERE kind='e027v2_cycle' AND key=?", (str(run_id),)
        )
    }
    all_started = {
        int(r["key"].split(":")[-1])
    ...
    summary = {
        ...
        "prior_run_usage": protocol["prior_run"]["usage"],
        "new_run_budget_is_separate": True,
        "prior_receipts_reused": False,
    ...
    rec = archive.append("e027v2_stopped", str(run_id), utcnow(), {}, canonical(summary).encode())
    summary["report_record_id"] = rec
    publish(report_dir / f"E027v2_stopped_{run_id}.json", summary)
    return summary
...
def main():
    ...
    try:
        with singleton_lock(archive.root):
            if args.register:
                ...
                publish(Path(args.report_dir) / "E027v2_registration.json", {**result, "protocol": protocol})
