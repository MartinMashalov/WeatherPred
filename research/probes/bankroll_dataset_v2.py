"""E024 v2: preserve v1 and recognize four explicitly pinned categorical results.
...
# Exception authorized after metadata-only diagnostic125267/report125381.
# No thermometer value is inferred from this categorical source string.
CATEGORICAL_EVENT = "KXHIGHMIA-26APR11"
CATEGORICAL_TICKERS = frozenset(
    CATEGORICAL_EVENT + "-" + suffix for suffix in ("T83", "T76", "B82.5", "B76.5")
)
CATEGORICAL_SOURCE_ID = 108042
CATEGORICAL_SOURCE_SHA256 = "ba2ae40b7157ec7e5d08ecd78a931f95f2c1cf95be979b657dd24f1c3d754021"
CATEGORICAL_CHECKPOINT_ID = 108049


def numeric_corroboration(market, predicate, provenance, checkpoint_record_id, eligible):
    value = market.get("expiration_value")
    if value in (None, ""):
        return "missing"
    if value == "No":
        if (
            market.get("event_ticker") != CATEGORICAL_EVENT
            or market.get("ticker") not in CATEGORICAL_TICKERS
            or provenance.get("source_record_id") != CATEGORICAL_SOURCE_ID
            or provenance.get("body_sha256") != CATEGORICAL_SOURCE_SHA256
            or checkpoint_record_id != CATEGORICAL_CHECKPOINT_ID
            or market.get("result") != "no"
            or market.get("status") not in ("finalized", "settled")
            or not eligible
        ):
            raise ValueError("Categorical expiration result lacks its exact declared source/result binding")
        return "categorical_result_only"
    try:
        numeric = Decimal(str(value))
    except (InvalidOperation, ValueError) as error:
        raise ValueError("Unregistered nonnumeric expiration value") from error
    if not numeric.is_finite():
        raise ValueError("Nonfinite expiration value")
    if eligible:
        if yes_at(predicate, value) != (market["result"] == "yes"):
            raise ValueError("Final settlement value contradicts primary contract predicate")
        return "numeric_value_matches_result"
    return "not_final_binary_with_settlement_time"
...
def market_metadata(market, event, provenance, windows, *, checkpoint_record_id=None):
    ...
    corroboration = numeric_corroboration(market, predicate, provenance, checkpoint_record_id, eligible)
    return {
        ...
        "numeric_corroboration": corroboration,
...
def normalize_event(reader, checkpoint, windows, *, historical_cutoff=None, checkpoint_record_id=None):
...
def build_dataset(reader, windows, amendment_protocol_ids=()):
    ...
    for identifier, checkpoint in checkpoints:
        for market in normalize_event(
            ...
            checkpoint,
            windows,
            historical_cutoff=config["cutoff"]["market_settled_ts"],
            checkpoint_record_id=identifier,
        ):
    ...
    return {
        ...
        "numeric_corroboration_counts": dict(
            Counter(m["metadata"]["numeric_corroboration"] for m in markets)
        ),
        "normalization_revision": "E024-v2-four-pinned-categorical-No-results",
        "actual_fills": 0,
        "historical_availability_verified": False,
